...
import com.velocitypowered.proxy.adventure.VelocityAdventure;
import com.velocitypowered.proxy.network.ConnectionState;
import com.velocitypowered.proxy.network.PacketProcessor;
import com.velocitypowered.proxy.network.packet.client.ClientPacket;
import com.velocitypowered.proxy.network.packet.client.handshake.HandshakePacket;
import com.velocitypowered.proxy.network.packet.proxy.*;
import com.velocitypowered.proxy.network.packet.proxy.login.SetCompressionPacket;
import com.velocitypowered.proxy.network.socket.Worker;
 ...
 */
@ApiStatus.Internal
public class ClientSocketConnection extends ClientConnection {

    private final static Logger LOGGER = LoggerFactory.getLogger(ClientSocketConnection.class);
    ...
    public ClientSocketConnection(@NotNull Worker worker, @NotNull SocketChannel channel, SocketAddress remoteAddress) {
        super();
        this.worker = worker;
    ...
    private void writePacketSync(SendablePacket packet, boolean compressed) {
        ...
        if (player != null && outgoing.hasListener()) {
            final ProxyPacket proxyPacket = SendablePacket.extractServerPacket(packet);
            PlayerPacketOutEvent event = new PlayerPacketOutEvent(player, proxyPacket);
            outgoing.call(event);
            if (event.isCancelled()) return;
        }
        // Write packet
        if (packet instanceof ProxyPacket proxyPacket) {
            writeServerPacketSync(proxyPacket, compressed);
    ...
    private void writeServerPacketSync(ProxyPacket proxyPacket, boolean compressed) {
        final Player player = getPlayer();
        if (player != null) {
            if (VelocityAdventure.AUTOMATIC_COMPONENT_TRANSLATION && proxyPacket instanceof ComponentHoldingProxyPacket) {
                proxyPacket = ((ComponentHoldingProxyPacket) proxyPacket).copyWithOperator(component ->
                        GlobalTranslator.render(component, Objects.requireNonNullElseGet(player.getLocale(), VelocityAdventure::getDefaultLocale)));
            }
        }
        try (var hold = ObjectPool.PACKET_POOL.hold()) {
            var buffer = PacketUtils.createFramedPacket(hold.get(), proxyPacket, compressed);
            writeBufferSync(buffer, 0, buffer.limit());
        }
    }
